...
- -`otype`:检测结果输出格式，输出后需要拷贝到其他地方请搭配sendfile_list参数使用
    ...
    - 0x2：标准m3u格式
    - 0x4：标准txt格式
    - 组合，以上可以组合输出多个格式，如：otype=7

- -`sendfile_list`:检测结果同步拷贝到目的地（不指定文件名则自动命名文件）
    - 文件的数量需与otype参数一致，例如，如果你在otype中选择了两个格式（如0x1和0x2），那么sendfile_list也必须包含两个文件路径。
    - 次序需与otype参数一致，也就是说，第一个文件路径对应otype中的第一个格式，第二个文件路径对应第二个格式，以此类推。
    - 如果你在otype中设置某个格式，但不想将其拷贝输出到文件中，可以在sendfile_list中将路径设置为""。
